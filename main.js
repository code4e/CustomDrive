(function () {
    ...
    function viewAlbum() {
        ...
        let album = resources.find(r => r.id == rid);
        ...
        spanAdd.addEventListener("click", function () { addImgFromURL(album); });

        let spanRight = divAlbumBody.querySelector("[purpose='picture-view']").querySelector(".move-right");
        let spanLeft = divAlbumBody.querySelector("[purpose='picture-view']").querySelector(".move-left");
        spanRight.addEventListener("click", function () { moveToRightImg(this, album.images); });
        spanLeft.addEventListener("click", function () { moveToLeftImg(this, album.images) });


        //load images when album is viewed
        
        album.images.forEach(image => {

            let imgPreview = document.createElement('img');
            imgPreview.setAttribute("src", image.imgSrc);

            imgPreview.onload = function () {

                //load HTML of each image
                addImgHTML(image.imgSrc, image.imgId, imgPreview);
    ...
    function moveToLeftImg(spanLeft, images) {
        let divPictureMain = spanLeft.nextElementSibling;
        let pictureMain = divPictureMain.querySelector("img");
        let imgId = parseInt(pictureMain.getAttribute("imgId"));
        let imgSrc = pictureMain.getAttribute("src");
        
        let divPictureList = spanLeft.parentNode.previousElementSibling;

        let pictureList = divPictureList.children;
        let prevImgIdx = 0;

        let idx = 0;
        for( ; idx < pictureList.length ; idx++){
            let previewImgId = parseInt(pictureList[idx].getAttribute("imgId"));
            if(previewImgId == imgId){
                break;
            }
        }
        ...
        if(prevImgIdx < 0) prevImgIdx = pictureList.length - 1;

        prevImgIdx %= pictureList.length;

        let prevImgId = images[prevImgIdx].imgId;
        let prevImgSrc = images[prevImgIdx].imgSrc;
        
        pictureViewInMain(prevImgIdx, prevImgSrc);

    }


    function moveToRightImg(spanRight, images) {


        let divPictureMain = spanRight.previousElementSibling;
        let pictureMain = divPictureMain.querySelector("img");
        let imgId = parseInt(pictureMain.getAttribute("imgId"));
        let imgSrc = pictureMain.getAttribute("src");

        let divPictureList = spanRight.parentNode.previousElementSibling;
        ...
        nextImgIdx = idx + 1;
        nextImgIdx %= pictureList.length;
        console.log(nextImgIdx);

        let nextImgId = images[nextImgIdx].imgId;
        let nextImgSrc = images[nextImgId].imgSrc;
        
        pictureViewInMain(nextImgId, nextImgSrc);
        

    }

    function addImgHTML(imgSrc, imgId, imgPreview) {

        let divAlbumBody = divAppBody.querySelector(".album-body");
        let divPictureList = divAlbumBody.querySelector("[purpose='picture-list']");

        //add to html
        imgPreview.setAttribute("imgId", imgId);
        divPictureList.appendChild(imgPreview);
        imgPreview.addEventListener("click", function () { pictureViewInMain(imgId, imgSrc); } );

        pictureViewInMain(imgId, imgSrc);
    }


    function addImgFromURL(album) {
        let imgSrc = prompt("Please enter a link to the image");
        if (!imgSrc) {
            alert("Please enter a valid URL");
            return;
        }
        ...
        imgPreview.onload = function () {
            //on successfull loading of img, add to ram
            images.push({
                imgId: cImgID,
                imgSrc: imgSrc
            });

            addImgHTML(imgSrc, cImgID, imgPreview);
    ...
    function pictureViewInMain(imgId, imgSrc) {


        let divPictureMain = divAppBody.querySelector("[purpose='album-body']").querySelector("[purpose='picture-view']").querySelector(".picture-main");
        let imgMain = document.createElement("img");
        imgMain.setAttribute("src", imgSrc);
        imgMain.setAttribute("imgId", imgId);

        divPictureMain.innerHTML = '';
        divPictureMain.appendChild(imgMain);
